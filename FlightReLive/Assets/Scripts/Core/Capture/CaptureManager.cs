...
namespace FlightReLive.Core.Capture
{
    public class CaptureManager : MonoBehaviour
    {
        ...
        internal void StartCapture()
        {
            ...
            if (volume == null)
            {
                GameObject go = new GameObject("CapturePassVolume");
            ...
            {
                Fugui.Notify("Critical capture error", "Unable to start capture recording.", StateType.Danger);
                return;
            }
        ...
        private string GetPlatformFFmpegPath()
        {
        ...
        #region CALLBACKS
        private void OnCaptureResolutionChanged(int resolution)
        {
            switch (resolution)
            {
                case 0:
        ...
        internal void DrawCaptureModeSettings(FuLayout layout)
        {
            using (FuGrid grid = new FuGrid("gridCaptureSettings", new FuGridDefinition(2, new float[2] { 0.3f, 0.7f }), FuGridFlag.AutoToolTipsOnLabels, rowsPadding: 3f, outterPadding: 10))
            {
                ...
                grid.InputFolder("Capture output path", (path) => SettingsManager.SaveCaptureOutputPath(path), captureOutputPath, new ExtensionFilter[0]);
