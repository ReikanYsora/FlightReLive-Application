...
namespace FlightReLive.Core.Pipeline.API
{
    public static class MapTilerAPIHelper
    {
        ...
        #region HILLSHADE
        internal static async Task DownloadHillShadeTilesParallelAsync(FlightData flightData, int zoomLevel, Action onTileDownloaded)
        {
            List<TileDefinition> tiles = flightData.MapDefinition.TileDefinitions;

            // Regrouper les tuiles enfants par tuile parent
            var groupedTiles = tiles
                .Where(tile => tile.ZoomLevel >= zoomLevel)
                .GroupBy(tile =>
                {
                    int zoomDiff = tile.ZoomLevel - zoomLevel;
            ...
            List<Task> activeTasks = new List<Task>();
            Queue<IGrouping<(int x, int y), TileDefinition>> queue = new Queue<IGrouping<(int x, int y), TileDefinition>>(groupedTiles);
            ...
            while (queue.Count > 0 || activeTasks.Count > 0)
            {
                while (activeTasks.Count < MAX_CONCURRENT_REQUEST && queue.Count > 0)
                {
                    ...
                    Task task = ProcessHillshadeGroupAsync(group.Key.x, group.Key.y, group.ToList(), zoomLevel, onTileDownloaded);
                    activeTasks.Add(task);
                }
        ...
        private static async Task ProcessHillshadeGroupAsync(int parentX, int parentY, List<TileDefinition> children, int sourceZoom, Action onTileDownloaded)
        {
            string url = $"https://api.maptiler.com/tiles/hillshade/{sourceZoom}/{parentX}/{parentY}.webp?key={SettingsManager.CurrentSettings.MapTilerAPIKey}";

            byte[] webpData = await DownloadWebP(url);
            ...
            Texture2D fullTexture = DecodeWebP(webpData);
            ...
            foreach (var tile in children)
            {
                if (await CacheManager.HillShadeTileExistsAsync(tile.X, tile.Y))
                {
                    tile.HillShadeTexture = await CacheManager.LoadHillShadeTileAsync(tile.X, tile.Y);
                    onTileDownloaded?.Invoke();
                    continue;
                }

                Texture2D hillShadeCrop = CropSubTileFromParentTexture(fullTexture, tile, sourceZoom);
                if (hillShadeCrop != null)
                {
                    await CacheManager.SaveHillShadeTileAsync(hillShadeCrop, tile.X, tile.Y);
                    tile.HillShadeTexture = hillShadeCrop;
                }
        ...
        private static async Task<byte[]> DownloadWebP(string url)
        {
            TaskCompletionSource<byte[]> tcs = new TaskCompletionSource<byte[]>();
            ...
            DownloadManager.EnqueueDownload(url,
                ...
                error =>
                {
                    Debug.LogWarning($"Hillshade tile download failed: {error}");
                    tcs.SetResult(null);
                });
        ...
        private static Texture2D DecodeWebP(byte[] webpData)
        {
            int width = MapTools.TILE_RESOLUTION;
            int height = MapTools.TILE_RESOLUTION;
            Error errorCode;
            ...
            {
                Debug.LogWarning($"WebP decoding exception: {ex.Message}");
                return null;
            }
        }

        private static Texture2D CropSubTileFromParentTexture(Texture2D fullTexture, TileDefinition tile, int sourceZoom, int padding = 0)
        {
            int zoomDiff = tile.ZoomLevel - sourceZoom;
            if (zoomDiff < 0) return null;

            int fullWidth = fullTexture.width;
            int fullHeight = fullTexture.height;

            int tilesPerAxis = 1 << zoomDiff;
            int subTileSize = fullWidth / tilesPerAxis;

            int localX = tile.X % tilesPerAxis;
            int localY = tile.Y % tilesPerAxis;
            int flippedLocalY = tilesPerAxis - 1 - localY;

            int startX = localX * subTileSize - padding;
            int startY = flippedLocalY * subTileSize - padding;
            int cropSize = subTileSize + padding * 2;

            if (startX < 0 || startY < 0 || startX + cropSize > fullWidth || startY + cropSize > fullHeight)
            {
                return null;
            }

            Color[] pixels = fullTexture.GetPixels(startX, startY, cropSize, cropSize);
